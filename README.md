...
- 🎯 Entry points to execute your program using `python -m <net_worth_tracker>` or `$ net_worth_tracker` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/bkm82/net-worth-tracker/branch/main/graph/badge.svg?token=net-worth-tracker_token_here)](https://codecov.io/gh/bkm82/net-worth-tracker)
[![CI](https://github.com/bkm82/net-worth-tracker/actions/workflows/main.yml/badge.svg)](https://github.com/bkm82/net-worth-tracker/actions/workflows/main.yml)

Awesome net_worth_tracker created by bkm82

## Install

from source
```bash
git clone https://github.com/bkm82/net-worth-tracker net_worth_tracker
cd net_worth_tracker
make install
```
...
```bash
pip install net_worth_tracker
```

## Executing

```bash
$ net_worth_tracker run --port 8080
```

or

```bash
python -m net_worth_tracker run --port 8080
```

or

```bash
$ uvicorn net_worth_tracker:app
```
...
Usage: net_worth_tracker [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ net_worth_tracker create-user --help
Usage: net_worth_tracker create-user [OPTIONS] USERNAME PASSWORD
...
```bash
net_worth_tracker create-user admin admin --superuser
```
...
Run with `net_worth_tracker run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `net_worth_tracker create-user`.
...
-----------------------------------------------------
net_worth_tracker/__init__.py              4      0   100%
net_worth_tracker/app.py                  16      1    94%
net_worth_tracker/cli.py                  21      0   100%
net_worth_tracker/config.py                5      0   100%
net_worth_tracker/db.py                   10      0   100%
net_worth_tracker/models/__init__.py       0      0   100%
net_worth_tracker/models/content.py       47      1    98%
net_worth_tracker/routes/__init__.py      11      0   100%
net_worth_tracker/routes/content.py       52     25    52%
net_worth_tracker/routes/security.py      15      1    93%
net_worth_tracker/routes/user.py          52     26    50%
net_worth_tracker/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in net_worth_tracker/default.toml.
